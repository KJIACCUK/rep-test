<?php
    ...
    class FilterHelper
    {
        ...
        public static function checkEventExists($eventId)
        {
            ...
            $criteria->addColumnCondition(array('eventId' => $eventId));
            $criteria->addCondition('status = "'.Event::STATUS_APPROVED.'" OR (status = "'.Event::STATUS_WAITING.'" AND userId = "'.$controller->getUser()->userId.'")');
            return (bool)Event::model()->find($criteria);
        }
        ...
        public static function checkRegistrationStep($step)
        {
            return in_array($step, array(1,2,3,4));
        }
